...
description: >-
  ...
  verifies and removes the work dir, and the note records the reclaimed bytes — plus the artifact-staging
...
which `log-experiment` restates on its final `OK:` line as `[scratch: …]`, and copy the number onto the
note's `scratch reaped:` line (Step 2). The outcomes and what each means:

| `[scratch: …]` | what happened | what you do |
|---|---|---|
| `SCRATCH-REAP-RECLAIMED: … bytes=N …` | archived, verified, deleted | record `bytes=N` on the note |
| `none` | no work dir at `<root>/<name>` | record `n/a` — nothing was staged on the box |
| `not-wired` / `SCRATCH-REAP-GAP: …` | a seam is unset, so nothing was archived **and nothing deleted** | record the gap line; the dir is exactly where it was — get the instance seam wired |
| `reap-FAILED` / `reaper-not-found` | the record is merged and durable; the **work dir is still on disk** | record it, then re-run the reap by hand once the printed cause is fixed |

A note with no `scratch reaped:` line is **visibly unfinished** — that is the whole point of putting it in
the skeleton. Because the record is already merged by the time the reap runs, a bad reap outcome never fails
the landing; it is reported, not swallowed.
...
- No new gates, no audit machinery for notes — the secret scan `log-experiment` already runs for any note is
  ...
  note, never enforced against the landing. What it does have is a fail-closed *delete* guard — it archives
  and verifies before removing anything, and refuses outright if it can't.
...
- **`templates/NOTE_TEMPLATE.md`** — the pinned `NOTE.md` skeleton (including the `scratch reaped:` line).
