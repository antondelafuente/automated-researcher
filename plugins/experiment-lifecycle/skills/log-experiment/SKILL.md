...
page source stages into the SAME commit and the SAME gated PR as the record. The close used to land the
record, then the page source, then `LANDED.md` as three sequential gated PRs (~10 min of a measured 51-min
median close leg) — the same close, gated the same way, three times. The dir must live in this repo, be
**physically** disjoint from the record dir (each root's real path must be inside the repo's real root, and
neither root may be an ancestor of the other or be the repo root itself — a symlink cannot make an
overlapping root look disjoint), and be **note-shaped** (no `DESIGN.md`/`RESULTS.md`/`KIND` of its own): it
rides the record's gate, so it must carry no evidence of its own to verify. `--page-source-only <path>`
narrows it exactly as `--only` narrows the record dir — a dashboard tree is typically multi-tenant, so a
co-tenant's untracked files must not sweep in (#374).
...
answer for a viewer living in a **different repo**, which no single PR can reach: it records where the page
source went instead of staging it, and is mutually exclusive with `--page-source`. Both are refused on a
non-experiment kind — the publish leg belongs to an experiment's close.
...
- **Design-stage.** The **pre-launch leg** of the two-PR flow (`DESIGN.md` present, no `RESULTS.md` yet). The
  ...
  as a `note` before this kind existed, so design-stage must not drop that scan. It **also verifies the
  `START.md` instance-profile snapshot** (`aar_profile_snapshot.sh check`, #469) — the single deterministic
  owner of that check: BLOCK unless `START.md` is present, carries the fenced-TOML `## Instance profile
  (snapshot)` block `design-experiment`'s Step 3 writes, and that block's `profile_sha256` still matches the
  live instance profile (not stale). A `[recipes.viewer]`-less snapshot is a legitimate manifest-only instance
