- experiment-lifecycle 0.4.5 / repo-janitor 0.3.4 (2026-08-31): sparse worktrees by default — the scaffold
...
- experiment-lifecycle 0.3.45 (2026-07-14): revive #195's aar-profile discovery + START.md snapshot helper
  ...
  `## Instance profile (snapshot)` block at all — the snapshot grammar was specified (#153, SCHEMA.md) but no
  script ever wrote or checked it, and `design-experiment` SKILL.md only mentioned it in a parenthetical
  aside. `snapshot <START.md>` resolves the live `aar-profile` by the standard discovery order, fails closed
  (`BLOCK: ...`) on no discoverable profile or an unknown `schema_version` MAJOR, and writes/replaces the
  fenced-TOML block with `[github]` + `[recipes.viewer]` only (never `[recipes.visualization_*]`, which
  `visualize-results` resolves live by design) plus a `profile_sha256` drift hash; re-running is idempotent
  (replaces in place). `check <START.md>` re-resolves the live profile and BLOCKs if the block is
  missing/unparseable, its `profile_sha256` no longer matches the live file (stale), or `[recipes.viewer]`'s
  presence disagrees between the block and the live profile — a viewer-less profile is a legitimate
  manifest-only instance and passes. `design-experiment` SKILL.md Step 3 gets an explicit item to run
  `snapshot` before the design-stage commit (once per START.md in a multi-arm wave, so every arm shares one
  `profile_sha256`); `START_TEMPLATE.md` ships the block's placeholder heading + an empty `toml` fence with a
  "generated by the helper — do not hand-write" note. `log-experiment.sh`'s `gate_design_stage` is the SINGLE
  deterministic enforcement owner: it now also requires `START.md` present and its snapshot to pass `check`,
  surfacing the check's one-line reason verbatim on BLOCK — design-audit stays scoped to data-trustability,
  this gate is purely mechanical. The helper ships as a byte-identical copy in both `design-experiment/scripts/`
  and `log-experiment/scripts/` (same per-skill-copy + `.aar-ci/checks.sh` drift-check precedent as the
  aar-profile `SCHEMA.md` and feedback-loop's init helper) since `design-experiment` writes the snapshot and
  `log-experiment`'s gate reads it back, and the two skills install independently. Executor (`run-experiment`)
  side needs no change — it already reads only the frozen snapshot (#449's rebuild-verified gate covers the
  publish leg once the pointer arrives). Adds `aar_profile_snapshot_smoke.sh` (round-trip: write, idempotent
  re-write, staleness, fail-closed on a missing/unknown-schema profile, the manifest-only path, a
  hand-tampered presence mismatch, copy-drift) and `log_experiment_design_stage_snapshot_smoke.sh` (the gate
  integration: valid snapshot passes, missing `START.md`/missing block/stale snapshot each BLOCK with the
  expected message, a legitimate viewer-less snapshot still passes) — both run by `.aar-ci/checks.sh` when
  the helper, the gate, or either smoke changes.
